...
1. `userChrome.css`に以下のCSSセレクターを定義する。（`userChrome.css`の適用方法は割愛します。）  
```css
...
2. `about:config`（`prefs.js`）より`browser.tabs.inTitlebar`の値を`0`に定義する。

3. [Firefox](https://www.mozilla.org/firefox/new/) を再起動する。再起動後、タブバーが非表示になっていれば成功です。（タブバー周辺の表示がおかしな場合は [参考文献](#参考文献) をご参照ください。）
