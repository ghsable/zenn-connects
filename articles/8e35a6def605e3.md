...
1. `about:config`（`prefs.js`）より`browser.tabs.inTitlebar`の値を`0`に定義する。（`Menu > More tools > Customize toolbar... > Title Bar`が`1（チェックありの状態）`と同義です。）

2. `userChrome.css`に以下のCSSセレクターを定義する。（`userChrome.css`の適用方法は割愛します。）  
```css
